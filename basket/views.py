...
def view_basket(request):
    if request.method == 'POST':
        ...
        if formset.is_valid():
            ...
            messages.error(
                request, "Votre panier n'a pas pu être mis à jour, veuillez en examiner un"
...
def add_to_basket(request, product_id):
    ...
    if new:
        ...
        if basket_item.quantity < 5:
            ...
            messages.warning(
                request, 'Vous avez la quantité maximale autorisée de cet article \
                    dans votre panier, aucun autre ne peut être ajouté.')
...
def get_basket(sender, user, request, **kwargs):
    ...
    try:
        ...
        if hasattr(request, 'basket'):
            ...
            messages.info(
                request, "Vos nouveaux articles ont été fusionnés avec votre \
                 panier existant.")
